      ...
      <nav className="relative z-20 border-b border-white/5">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-6">
            ...
            <button onClick={() => router.push("/login")} className="text-xs px-4 py-2 border border-[#C9A84C]/50 text-[#C9A84C] rounded-lg hover:bg-[#C9A84C]/10 transition-colors tracking-wide">
              Client Login
            </button>
          </div>
        </div>
      ...
      <section className="relative z-10 max-w-6xl mx-auto px-6 pt-24 pb-20">
        <div className="max-w-3xl">
          ...
          <p className="text-lg text-slate-400 leading-relaxed mb-10 max-w-2xl">
            Sovereign Vault captures your technical activity in a cryptographically verified blockchain ledger — producing regulator-ready evidence that withstands HMRC, IRS, and ZATCA scrutiny.
          </p>
      ...
      <section id="calculator" className="relative z-10 bg-[#0a1220]/60 border-y border-white/5">
        <div className="max-w-6xl mx-auto px-6 py-24">
          <div className="mb-12">
            ...
            <p className="mt-3 text-sm text-slate-500 max-w-lg">Enter your technical spend and we'll calculate your estimated recovery value across all four regimes in real time.</p>
          </div>
            ...
            <div className="bg-[#0d1520] border border-[#1e3a5f]/60 rounded-2xl p-8 space-y-5">
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {JURISDICTIONS.map((j) => (
                    <button key={j.value} type="button" onClick={() => setJurisdiction(j.value)}
                      className={`py-2.5 px-3 rounded-lg border text-xs text-left transition-colors ${jurisdiction === j.value ? "border-[#C9A84C]/60 bg-[#C9A84C]/8 text-[#C9A84C]" : "border-[#1e3a5f] text-slate-400 hover:border-[#1e3a5f]/80"}`}>
                      <div>{j.label}</div>
              ...
              <div>
                <label className="block text-[11px] uppercase tracking-widest text-slate-500 mb-2">Annual Technical Spend ({selectedJurisdiction.regime})</label>
                <div className="relative">
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 text-sm">
                    {jurisdiction === "UK" ? "£" : jurisdiction === "USA" ? "$" : jurisdiction === "UAE" ? "AED" : "SAR"}
                  </span>
                  <input type="number" value={spend} onChange={(e) => setSpend(e.target.value)} placeholder={selectedJurisdiction.placeholder}
    ...
    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-52 p-2 bg-slate-800 text-xs text-slate-300 rounded shadow-xl border border-slate-700 opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity z-50">
              ...
              {result && (
                <div className={`flex-1 border rounded-2xl p-8 ${tierBg(result.tier)}`}>
                  <div className="flex items-center justify-between mb-6">
                    <div className={`text-[11px] uppercase tracking-widest font-semibold ${tierColour(result.tier)}`}>{result.tier} CASE</div>
                    <div className={`text-[11px] px-3 py-1 rounded-full border ${result.proceed ? "border-emerald-400/40 text-emerald-400" : "border-red-400/40 text-red-400"}`}>
                  ...
                  {result.proceed && (
                    <button type="button" onClick={() => router.push("/generate-key")}
      ...
      <section className="relative z-10 border-t border-white/5">
        <div className="max-w-6xl mx-auto px-6 py-24 text-center">
          ...
          <div className="flex flex-wrap gap-4 justify-center">
            ...
            <button onClick={() => router.push("/login")} className="px-8 py-3.5 border border-white/15 text-slate-300 text-sm rounded-lg hover:bg-white/5 transition-colors tracking-wide">
              Client Login
            </button>
          </div>
        </div>
      ...
      <footer className="relative z-10 border-t border-white/5">
        <div className="max-w-6xl mx-auto px-6 py-8 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="text-[11px] text-slate-600">This platform does not constitute tax advice. Consult a qualified R&D tax specialist before filing.</div>
        </div>
      </footer>
