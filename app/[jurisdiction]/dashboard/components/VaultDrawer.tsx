...
export function VaultDrawer({ open, clientHash, vaultName, onClose }: VaultDrawerProps) {
  ...
  const formattedAccrual = accrual
    ? new Intl.NumberFormat(accrual.locale, { style: "currency", currency: accrual.currency, maximumFractionDigits: 0 }).format(accrual.accrual_value)
    : null;
  const formattedRate = accrual
    ? new Intl.NumberFormat(accrual.locale, { style: "currency", currency: accrual.currency, maximumFractionDigits: 0 }).format(accrual.rate_per_signal)
    : null;
  return (
    <>
      <div aria-hidden="true" onClick={onClose} className={`fixed inset-0 z-40 bg-black transition-opacity duration-300 ${open ? "opacity-60" : "opacity-0 pointer-events-none"}`} />
      <div ref={drawerRef} role="dialog" aria-modal="true" aria-label="Vault activation" className={`fixed top-0 right-0 h-full w-full max-w-md z-50 bg-[#0a0f1a] border-l border-[#1e3a5f] shadow-2xl flex flex-col transition-transform duration-300 ease-out ${open ? "translate-x-0" : "translate-x-full"}`}>
        <div className="flex items-center justify-between px-6 py-5 border-b border-white/[0.08]">
          <div>
            <div className="text-[10px] tracking-[0.3em] uppercase text-[#C9A84C]">Vault Access</div>
            <div className="text-sm text-white mt-0.5 font-medium truncate max-w-[260px]">{vaultName ?? clientHash?.slice(0, 24) + "..."}</div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
          ...
          <div className="bg-[#0d1520] border border-[#1e3a5f] rounded-xl p-5">
            ...
            {loading ? (
              ...
              <div className="text-sm text-slate-500 mt-1">Unable to load accrual data.</div>
            )}
          </div>
          <div>
          ...
          <div className="bg-[#0d1520] border border-[#1e3a5f]/60 rounded-xl p-4 text-xs text-slate-400 leading-relaxed">
            ...
            2500/mo + 20% Success Fee Management. Cancel anytime before your filing date.
          </div>
        </div>
        <div className="px-6 py-5 border-t border-white/[0.08] space-y-3">
          ...
          <button type="button" onClick={handleUnlock} disabled={paying} className="w-full py-3.5 bg-[#C9A84C] text-black text-sm font-semibold rounded-xl hover:bg-[#d4b560] disabled:opacity-60 disabled:cursor-not-allowed transition-colors tracking-wide">
            {paying ? "Redirecting to payment..." : "Unlock Unlimited Access"}
          </button>
          <p className="text-[10px] text-slate-600 text-center">2500/mo + 20% Success Fee Management.</p>
        </div>
      </div>
    </>
  );
